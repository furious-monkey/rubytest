...
class Reader
  ...
  # this method with read in the albums and the ids of the songs that belong to them from 
  # songs.csv file.
  # returns a hash table where the keys are the albums names and values is the song-ids (an array)

  def read_albums(csv_file_name, temp_hash = Hash.new { |hsh, key| hsh[key] = [] } )
    CSV.foreach(csv_file_name, :headers => true) do |row|

      songname, artist, album, time, id = row[0],row[1], row[2], row[3], row[4]
      unless (id =~ /#/)
        temp_hash[id] << [album, artist]
      end
    end
    temp_hash   
    p temp_hash
